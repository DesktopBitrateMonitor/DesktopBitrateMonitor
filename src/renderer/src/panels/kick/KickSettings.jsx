...
const KickSettings = () => {
  ...
  const kickBasePath = useMemo(() => {
    const parts = location.pathname.split('/').filter(Boolean);
    const kickIndex = parts.indexOf('kick');

    if (kickIndex === -1) return location.pathname;

    return `/${parts.slice(0, kickIndex + 1).join('/')}`;
  }, [location.pathname]);
  ...
  const activeValue = useMemo(() => {
    ...
    // When at /dashboard/accountssettings/kick (index route), default to commands
    if (last === 'kick' || last === 'kicksettings') return 'commandsettings';
  ...
  const handleChange = (_event, newValue) => {
    ...
    if (newValue === 'commandsettings') {
      navigate(kickBasePath, { replace: false });
    } else {
      navigate(`${kickBasePath}/${newValue}`, { replace: false });
    }
  };
